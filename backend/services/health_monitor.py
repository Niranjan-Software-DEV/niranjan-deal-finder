...
async def run_health_check() -> list[HealthCheckResult]:
    ...
    async def limited_check(scraper, query: str) -> HealthCheckResult:
        async with semaphore:
            return await _check(scraper, query)

    return list(await asyncio.gather(*[limited_check(s, q) for s, q in _SCRAPERS]))
