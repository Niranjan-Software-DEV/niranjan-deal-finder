...
@pytest.mark.asyncio
async def test_base_scraper_caps_products_before_caching(monkeypatch):
    ...
    async def fetch_html_stub(*args, **kwargs):
        return "html"

    monkeypatch.setattr("scrapers.base.cache_module.get", lambda *args, **kwargs: None)
    monkeypatch.setattr("scrapers.base.cache_module.store", lambda source, query, data: stored.append(data))
    monkeypatch.setattr("scrapers.base.fetch_html", fetch_html_stub)

    result = await DummyScraper().search("many products")

    assert len(result.products) == 20
    assert len(stored) == 1
    assert len(stored[0]) == 20
