...
git clone https://github.com/Niranjan-Software-DEV/niranjan-deal-finder.git
cd niranjan-deal-finder
...
Full list with descriptions. Set these in Render's dashboard (never commit real values). The Render blueprint uses `backend/Dockerfile`, which installs Chromium and runs the shared Playwright fallback as a single worker process. Browser fallbacks are serialized, and each marketplace is capped at 20 retained products to reduce peak memory on small instances.
...
| `CACHE_DIR` | — | `.cache` | JSON-safe SQLite cache directory used when Redis is not configured |
...
| `CONCURRENT_SCRAPE_LIMIT` | — | `2` | Max parallel scrapers; keep low on small Render instances |
| `MAX_PRODUCTS_PER_SOURCE` | — | `20` | Maximum products retained per marketplace result |
...
2. Command: `curl -fsS -H "X-Ops-Token: $OPS_TOKEN" $BACKEND_URL/api/v1/health`
...
If you need more:
- **Increase `CONCURRENT_SCRAPE_LIMIT`** only after watching memory metrics; the default is intentionally 2 on small Render instances
