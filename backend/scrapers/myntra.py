...
class MyntraScraper:
    ...
    def _products_from_cache(cached: cache_module.CacheEntry) -> list[Product]:
        ...
        for item in cached.data[:get_settings().max_products_per_source]:
            try:
    ...
    def _find_product_dicts(value) -> list[dict]:
        ...
        found: list[dict] = []
        max_found = max(get_settings().max_products_per_source * 3, 60)
    ...
    def _products_from_items(self, items: list[dict]) -> list[Product]:
        ...
        return products[:get_settings().max_products_per_source]
    ...
    def _parse_html(self, html: str, query: str | None = None) -> list[Product]:
        ...
        products: list[Product] = []
        for card in cards[:get_settings().max_products_per_source]:
        ...
        parsed = self._products_from_items([p.model_dump() for p in products[:get_settings().max_products_per_source]])
        if parsed:
